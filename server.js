...
app.get('/player_info/select_player', function(req, res) {
	// Param is the data in the url req.query.id
	var p_choice = req.query.player_choice;
	var names_ids = 'SELECT id,name FROM football_players;';
	var specific_player = 'SELECT * FROM football_players WHERE id=' + p_choice + ';';
	var total_player_games = "SELECT COUNT(*) FROM football_games INNER JOIN football_players ON football_players.id = ANY (football_games.players) WHERE football_players.id = " + p_choice + ";";

	db.task('get-everything', task => {
	    return task.batch([
		task.any(names_ids),
		task.any(specific_player),
		task.any(total_player_games)
	    ]);
	})
	.then(data => {
		res.render('pages/player_info',{
